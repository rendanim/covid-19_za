...
sa_data_2 = total_cases[67:85,c("South.Africa","date")]
...
sa_data_2$ds = as.Date(sa_data$ds , format = "%d/%m/%Y")
...
sa_model = prophet(sa_data_2,n.changepoints=5)
```
...
```{r}
sa_forecast_exp = exp(sa_forecast[c( 'yhat', 'yhat_lower', 'yhat_upper')])

```
```{r}
sa_future$time <- seq.int(nrow(sa_future))
sa_data$time <- seq.int(nrow(sa_data))
sa_data_2$time <- seq.int(nrow(sa_data_2))
...
trend.0 = trend_LM$new(D=1)
gpk.exp = GauPro_kernel_model$new(sa_data$time, sa_data$y, kernel=kern.exp, trend=trend.0, parallel=FALSE)
plot(gpk.exp)
```
```{r}
gp_pred = predict(gpk.exp, sa_future$time,se.fit = T)*population[5,2]/1000000

gp_plot_data =cbind(sa_future,gp_pred)
```


```{r}
ggplot(gp_plot_data, aes(x = ds, y = mean)) + 
  geom_line(color='darkblue', size = 1) +
    xlab("Date") + ylab("Case Forecast")+
  ggtitle("Gaussian Process Forecasts")+
    geom_ribbon(data=gp_plot_data,aes(ymin=mean-se,ymax=mean+se),fill='darkblue',alpha=0.3)+
theme_minimal()
 

```

##Aggregation of all Forecasts

```{r}
Agg_table =cbind(sa_future$ds,as.data.frame(south_africa_2o_predictions$forecast),as.data.frame(china_2o_predictions$forecast),as.data.frame(iran_2o_predictions$forecast),as.data.frame(sa_forecast$yhat),as.data.frame(gp_pred$mean))
colnames(Agg_table)=c("date","Polynomial ZA","Polynomial China","Polynomial Iran","FB Prophet","Gaussian Process")
kable(Agg_table[8:79,])
```
